...
namespace 作业2
{
    ...
    class Work4
    {
        ...
        static int[] S = new int[9] { 99, 99, 99, 99, 99, 99, 99, 99, 99 };//最短路径的顶点集合
        ...
        static void ShortestPathByDijkstra()
        {
            ...
            for (int f = 8; f >=0; f--)
            {
                ...
                next = 0;
                //找出第一行最小的列值
                for (int j = 0; j < 9; j++)//循环第0行的列
                {
                    if ((IsContain(j) == -1) && (graph[1, j] < min))//不在S中,找出第一行最小的元素所在的列
                    {
                        min = graph[1, j];
                        next = j;
                    }
                ...
                if (min != 10000)
                {
                    ...
                    Console.WriteLine("V1到V{0}的最短路径为：{1},路径为：V1{2}->V{0}", next, id1, mid[next]);
                }
                // 重新初始0行所有列值
                for (int j = 0; j < 9; j++)//循环第0行的列
                {
                    ...
                    {
                         if ((graph[next, j] + min) < graph[1, j])//如果小于原来的值就替换
                        {
                            if(j!=1)
                        {
                            graph[1, j] = graph[next, j] + min;
            ...
            for (int i = 0; i < 9; i++)
            {
                if (IsContain(i) == -1 && i != 1)
                    Console.WriteLine("V1到V{0}的最短路径为：无", i);
